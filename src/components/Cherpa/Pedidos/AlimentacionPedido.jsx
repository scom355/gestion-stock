...
const AlimentacionPedido = ({ API_BASE, onAddToBucket, color }) => {
    const [catalogItems, setCatalogItems] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [quantities, setStockQuantities] = useState({});
    ...
    const fetchCatalog = async () => {
        setIsLoading(true);
        try {
            const res = await fetch(`${API_BASE}/catalog/seca`);
            if (res.ok) {
    ...
    const handleAdd = (p) => {
        ...
        const label = isNaN(qty) ? qty : `${qty} UNID/CAJA`;
        onAddToBucket(p.name, label);
        setStockQuantities({ ...quantities, [p.id]: '' });
    };
                    ...
                    <table className="stock-table">
                        ...
                        <tbody>
                            {catalogItems.map((p, idx) => (
                                <tr key={p.id}>
                                    <td style={{ fontSize: '11px', textAlign: 'center', fontWeight: '900', color: '#94a3b8' }}>{idx + 1}</td>
                                    ...
                                    <td>
                                        <input 
                                            ...
                                            onChange={(e) => setStockQuantities({ ...quantities, [p.id]: e.target.value })} 
                                            className="qty-input" 
                                            onBlur={() => handleAdd(p)}
                                        />
                                    </td>
...
export default AlimentacionPedido;
